 ...
 * Uses file I/O which is a more realistic workload for Gluten/Velox.
 *
 * To run:
 *   ./build/sbt "runMain org.apache.gluten.benchmark.aggregate.ParquetAggregateBenchmark"
 */
object ParquetAggregateBenchmark extends GlutenBenchmarkBase {
  ...
  override def benchmarks: Seq[BenchmarkDef] = Seq(
    ...
    BenchmarkDef("Parquet Multi-agg GROUP BY", N, spark =>
      spark.read.parquet(dataPath)
        .groupBy("key_low")
        .agg(
          "id" -> "sum",
    ...
    BenchmarkDef("Parquet Filter + SUM", N, spark =>
      spark.read.parquet(dataPath)
        .filter("id > 50000000")
        .selectExpr("sum(id)")
    ),
    ...
    // String aggregation
    BenchmarkDef("Parquet COUNT DISTINCT str", N, spark =>
      spark.read.parquet(dataPath)
